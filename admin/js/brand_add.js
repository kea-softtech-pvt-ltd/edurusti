function validateForm() 
{
	...
	$.ajax({
		...
		type: "POST",
		data: { cate_name : _cate_name, action:'model_exit'},//to pass the value on another page 
		success: function(result){
          if(result.trim()=='1'){
				$('#cate_name_error').show();
				$('#cate_name_error').html('Brand name already exit.');
				setTimeout(function(){ $('#cate_name_error').hide(); }, 3000);
				return false;
		  }
		  else {
...
//validateForm_update
function validateForm_update_model() 
{
	var _invalid = false;
	...
	if(!_invalid)
	{
		...
		url: "ajax/ajax_mobile_repair.php",  //for data update to database"
		type: "POST",
		data: { brand_id : brand_id,cate_name : _cate_name, action:'model_exit_update'},//to pass the value on another page 
		success: function(result){
          if(result.trim()=='1'){
				$('#cate_name_error').show();
				$('#cate_name_error').html('Brand name already exit.');
				...
				$.ajax({
					...
					formData.append('action', 'update_cate_insert');
					$.ajax({
						url: "ajax/ajax_mobile_repair.php", //for data insertion to database"
						type: "POST",
						data: formData,
